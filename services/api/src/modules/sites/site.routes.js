...
sitesRouter.get("/", (0, _rbac.requirePermission)("site.read"), (0, _errorHandler.asyncHandler)(async (req, res) => {
  const {
    role,
    userId
  } = req.auth;
  ...
  if (role === "WORKER") {
    ...
    const site = await _site.SiteService.getById(profile.currentSite.toString());
    const body = {
      success: true,
      data: [site]
    };
...
sitesRouter.get("/:id", (0, _rbac.requirePermission)("site.read"), (0, _errorHandler.asyncHandler)(async (req, res) => {
  const {
    role,
    userId
  } = req.auth;
...
sitesRouter.patch("/:id", (0, _rbac.requirePermission)("site.update"), (0, _errorHandler.asyncHandler)(async (req, res) => {
  const input = updateSiteSchema.parse(req.body);
  const site = await _site.SiteService.update(req.params.id, input, req.auth.userId);
  (0, _gateway.getIO)().to(_sharedTypes.ROOMS.admin()).to(_sharedTypes.ROOMS.site(site._id.toString())).emit(_sharedTypes.SOCKET_EVENTS.SITE_UPDATED, {
    siteId: site._id.toString()
  });
  const body = {
    success: true,
...
sitesRouter.delete("/:id", (0, _rbac.requirePermission)("site.delete"), (0, _errorHandler.asyncHandler)(async (req, res) => {
  const result = await _site.SiteService.delete(req.params.id, req.auth.userId);
  const body = {
    success: true,
    data: result
  };
