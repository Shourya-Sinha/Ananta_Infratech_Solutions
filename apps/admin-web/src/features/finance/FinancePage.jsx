...
export function FinancePage() {
  ...
  const addInvestment = useMutation({
    ...
    onSuccess: (_data, body) => {
      ...
      toast.success(`Investment of ${formatINR(Number(body.amountRupees))} added to ${siteName(site)} — totals and profit/loss updated.`);
    },
  ...
  // Capital (site.capital.manage) — money put in as capital, distinct from investments
  ...
  const addCapital = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(`Capital added to ${siteName(site)} — company totals updated.`);
    },
    onError: (err) => toast.error(err instanceof Error ? err.message : "Could not save capital."),
  });

  const reverseTransaction = useMutation({
    mutationFn: async (transactionId) => unwrap(api.post(`/finance/transactions/${transactionId}/reverse`)),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["finance"] });
      toast.success("Transaction reversed. An opposite entry was recorded.");
    },
    onError: (err) => toast.error(err instanceof Error ? err.message : "Could not reverse transaction."),
  });
  ...
  const transactionColumns = [
    ...
    { header: "Description", cell: (t) => (t.reversalOf ? "Reversal entry" : t.description ?? "—"), className: "font-body" },
    {
      ...
      cell: (t) =>
        ...
        ) : reversedTransactionIds.has(String(t._id)) ? (
          <span className="text-xs uppercase tracking-wide text-graphite-300">reversed</span>
        ) : (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              if (window.confirm("Reverse this transaction? An opposite entry will be recorded and profit/loss recalculated.")) {
                reverseTransaction.mutate(t._id);
              }
            }}
  ...
  const investmentColumns = [
    ...
    { header: "Note", cell: (i) => (i.reversalOf ? "Reversal entry" : i.note ?? "—"), className: "font-body" },
    {
      ...
      cell: (i) =>
        i.reversalOf ? (
          <span className="text-xs uppercase tracking-wide text-graphite-300">reversed</span>
        ) : reversedInvestmentIds.has(String(i._id)) ? (
          <span className="text-xs uppercase tracking-wide text-graphite-300">reversed</span>
        ) : (
          <button
            className="text-xs font-medium text-rust hover:underline disabled:opacity-50"
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-graphite-500">Investments, capital, income, expenses, and profit/loss — per site and company-wide gross.</p>
      </div>

      {companyScope && companyPnl && (
        <div>
          <p className="mb-2 font-display text-xs font-semibold uppercase tracking-wide text-graphite-500">Company-wide (all sites)</p>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <KpiCard label="Net profit / loss" value={formatINR(companyPnl.totalProfit - companyPnl.totalLoss)} tone={companyPnl.totalProfit - companyPnl.totalLoss >= 0 ? "positive" : "negative"} />
          </div>
          <p className="mt-2 text-xs text-graphite-500">Select a site below to see its individual income, expenses, and transaction history.</p>
        </div>
      )}
      ...
      {summaryLoading ? (
        ...
        totals && (
          <div className="space-y-3">
            <div>
              <p className="mb-2 font-display text-xs font-semibold uppercase tracking-wide text-graphite-500">Company gross summary — all {totals.siteCount} sites</p>
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              <p className="mt-2 text-xs text-graphite-500">
                Net position after recovering investments: <span className={cx("font-mono font-medium", totals.netPosition >= 0 ? "text-teal" : "text-rust")}>{formatINR(totals.netPosition)}</span> (income − expenses − investment)
              </p>
            </div>
                  ...
                  <thead>
                    <tr className="border-b border-steel-200 bg-steel-100/50">
                      {["Site", "Status", "Total investment", "Income", "Total expenses", "Profit / Loss", "Net position"].map((h) => (
                        <th key={h} className="whitespace-nowrap px-4 py-2.5 text-left font-display text-xs font-semibold uppercase tracking-wide text-graphite-500">
                          {h}
                        </th>
                      ))}
                    </tr>
                    ...
                    {siteRows.map((s) => (
                      <tr key={s.siteId} className="transition-colors duration-150 hover:bg-steel-100/40">
                        <td className="whitespace-nowrap px-4 py-3">
                          <span className="font-medium text-graphite-900">{s.siteName}</span> <span className="text-xs text-graphite-300">({s.siteCode})</span>
                        </td>
                  ...
                  {siteRows.length > 0 && (
                    <tfoot>
                      <tr className="border-t border-steel-300 bg-steel-100/70 font-semibold">
                        <td className="px-4 py-2.5 text-sm text-graphite-900" colSpan={2}>Gross total (all sites)</td>
        ...
        <div className="flex rounded border border-steel-200 bg-surface p-0.5">
          {TABS.map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`rounded px-3 py-1.5 text-sm transition-colors duration-150 ${tab === t ? "bg-graphite-900 text-paper" : "text-graphite-500 hover:bg-steel-100"}`}
            >
      ...
      {!site ? (
        <p className="rounded border border-dashed border-steel-200 p-6 text-center text-sm text-graphite-500">Select a site to view and manage its finances.</p>
      ) : tab === "profit-loss" ? (
        profitLoss && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
              ...
              <KpiCard label="Capital invested" value={formatINR(profitLoss.capitalInvested ?? 0)} sublabel="Via capital entries" />
              ...
              <KpiCard label="Total expenses" value={formatINR(profitLoss.totalExpenses)} sublabel="Material + labour" tone="negative" />
              <KpiCard label="Profit / Loss" value={formatINR(profitLoss.profitLoss)} sublabel={`Net position after investment: ${formatINR(profitLoss.netPosition)}`} tone={profitLoss.profitLoss >= 0 ? "positive" : "negative"} />
            </div>

            <div className="card p-4">
              <div className="flex items-center justify-between">
                <p className="font-display text-xs font-semibold uppercase tracking-wide text-graphite-500">Capital — site.capital.manage</p>
                <button className="btn-ghost !py-1 text-xs" onClick={() => setShowCapitalForm((v) => !v)}>
                  {showCapitalForm ? "Cancel" : "Add capital"}
                </button>
              </div>
              <p className="mt-1 text-xs text-graphite-500">Capital is money put in as owner capital (separate from investment). It is included in the company-wide capital total.</p>
              {showCapitalForm && (
                <form
                  ...
                  onSubmit={(e) => {
                    ...
                    addCapital.mutate({
                      siteId: site,
                      amountRupees: Number(capitalForm.amountRupees),
                      source: capitalForm.source,
                      date: capitalForm.date,
                      reference: capitalForm.reference || undefined,
                    });
                  }}
                >
                  ...
                  <input className="input" placeholder="Source (e.g. Owner, Bank)" required value={capitalForm.source} onChange={(e) => setCapitalForm({ ...capitalForm, source: e.target.value })} />
                  ...
                  <input className="input" placeholder="Reference (optional)" value={capitalForm.reference} onChange={(e) => setCapitalForm({ ...capitalForm, reference: e.target.value })} />
                  <button type="submit" className="btn-primary col-span-4 w-fit" disabled={addCapital.isPending}>
                    {addCapital.isPending ? "Saving…" : "Save capital"}
                  </button>
                </form>
              )}
            </div>
        ...
        <>
          {showInvForm && (
            ...
            >
              ...
              <input className="input" placeholder="Reference (optional)" value={invForm.reference} onChange={(e) => setInvForm({ ...invForm, reference: e.target.value })} />
              <input className="input" placeholder="Note (optional)" value={invForm.note} onChange={(e) => setInvForm({ ...invForm, note: e.target.value })} />
          ...
          <DataTable columns={investmentColumns} rows={investments} isLoading={investmentsLoading} emptyTitle="No investments recorded" emptyBody="Use “Add investment” to record money or material the admin has put into this site." />
        </>
      ) : (
        <>
          {showForm && (
          ...
          <DataTable columns={transactionColumns} rows={transactions} isLoading={isLoading} emptyTitle={`No ${tab} recorded`} emptyBody={`Record the first ${tab === "income" ? "income entry" : "expense"} for this site.`} />
          <p className="text-xs text-graphite-400">Tip: “Reverse” does not delete the record — it posts an opposite entry so ledgers stay immutable, like a financial reversal.</p>
        </>
      )}
    </div>
  );
